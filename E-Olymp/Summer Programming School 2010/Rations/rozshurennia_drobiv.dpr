...
function inp(var a:long):longword;
var
  i,j,n:longword;
begin
  i:=0;
...
procedure div_dec(a,b:long;n:longword);
var
  grb,s,q,r:long;
  i,k,k2,k5,f,en:word;
  p:integer;
    ...
    begin
      ...
      if not comp_0(r) then
        begin
          ...
          until (en=n)or(comp_abs(a,r)=0);
        end;
      if f<>0 then writeln;
      if en=0 then writeln('This expansion terminates.')else
        writeln('The last ',en,' digits repeat forever.');
    end else
      begin
        writeln('.0');
        writeln('The last 1 digits repeat forever.');
      end;
end;
...
begin
  ...
  while not eof(input)do
    begin
      ...
      n:=inp(b);
      readln;
      if not comp_0(a) or  not comp_0(b) then
        div_dec(a,b,n);
    end;
  close(input);
  close(output);
