...
bool find_ans(int prod, int sum, vector<int> &st, vector<int> &ans)
{
    if (mem[prod].find(sum) != mem[prod].end())
    {
        return false;
    }
    if (prod == 1 && sum == 0)
    ...
    for (int i = min(sum, prod); i >= 1; --i)
    {
        ...
        st.push_back(i);
        if (find_ans(prod / i, sum - i, st, ans))
        {
            return true;
        }
        st.pop_back();
